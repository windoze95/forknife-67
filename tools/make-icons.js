 ...
 *
 * Edit the colours and geometry HERE, not in public/icons/ — icon.svg is
 * generated by this file too, so hand-edits there are silently overwritten.
 *
 * Regenerate on **Node 22**, the version CI checks against:
 *
 *   npx -y node@22 tools/make-icons.js
 *
 * The pixels are deterministic but the PNG bytes are not: zlib ships with Node
 * and different Node lines compress the same input differently, so a run on
 * another version produces a diff that fails CI while changing nothing you can
 * see.
 */
...
/* Loot llama: the piñata's magenta mane into its lilac paper, crown in gold. */
const MANE = [255, 61, 154];
const LILAC = [181, 123, 255];
...
function renderIcon(size, { maskable = false } = {}) {
  ...
  for (let py = 0; py < size; py += 1) {
    for (let px = 0; px < size; px += 1) {
      ...
      const gradient = mix(MANE, LILAC, (px / size) * 0.5 + (py / size) * 0.5);
